...
- 统一的请求/响应日志记录
- 支持 allure 报告
- 支持注入已有的 BaseRequests 实例（session 复用）
"""
...
class BaseApi:
    ...
    def __init__(
        ...
        requests: Optional[BaseRequests] = None,
    ):
        """
        初始化 API 基类

        Args:
            base_url: 基础 URL，默认从 config 读取
            timeout: 请求超时时间
            enable_plugins: 是否启用插件系统
            requests: 外部传入的 BaseRequests 实例（用于 session 复用）
        """
        ...
        if requests:
            ...
            self.requests = BaseRequests(
                base_url=base_url, enable_plugins=enable_plugins
            )
    ...
    def post(
        ...
        response = self.requests.post(url, json=json_data, **kwargs)
    ...
    def put(
        ...
        response = self.requests.put(url, json=json_data, **kwargs)
    ...
    def delete(self, url: str, **kwargs) -> Dict[str, Any]:
        ...
        response = self.requests.delete(url, **kwargs)
